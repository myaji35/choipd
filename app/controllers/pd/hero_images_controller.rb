class Pd::HeroImagesController < Pd::BaseController
  ...
  def create
    ...
    if @hero_image.save
      redirect_to pd_hero_images_path, notice: "히어로 이미지가 추가되었습니다."
    else
  ...
  def update
    if @hero_image.update(hero_image_params)
      redirect_to pd_hero_images_path, notice: "히어로 이미지가 수정되었습니다."
    else
  ...
  def destroy
    @hero_image.destroy
    redirect_to pd_hero_images_path, notice: "히어로 이미지가 삭제되었습니다."
  end
